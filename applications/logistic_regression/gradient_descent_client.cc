...
int ExecuteProtocol() {
  ...
  std::vector<uint64_t> share_y = internal::SampleShareOfZero(
	    ...
	    gd_params.modulus).value().first;
 	std::vector<uint64_t> share_theta = internal::SampleShareOfZero(
  ...
  // Initialize LogRegShareProvider with preprocessed shares.
    ...
    share_provider  = 
    std::make_unique<LogRegShareProvider>(LogRegShareProvider::CreateNewMic(
        std::get<0>(vector_a_masks_x), beaver_triple_matrix_b_c_round1_p0,
        std::move(sigmoid_p0_new_mic), std::get<0>(vector_a_masks_x_transpose),
        beaver_triple_matrix_b_c_round3_p0, gd_params.num_examples,
        gd_params.num_features, gd_params.num_iterations).value());
	} else {
		// Initialize LogRegShareProvider with preprocessed shares.
    share_provider  = 
    std::make_unique<LogRegShareProvider>(LogRegShareProvider::Create(
        std::get<0>(vector_a_masks_x), beaver_triple_matrix_b_c_round1_p0,
  ...
  std::unique_ptr<GradientDescentPartyZero> gradient_descent_party_zero = 
   ...
   MaskedXTransposeMessage client_masked_x_transpose_message;
	...
	GradientDescentClientMessage client_message;
	GradientDescentServerMessage server_message;
  ...
  std::unique_ptr<GradientDescentRpc::Stub> stub =
      GradientDescentRpc::NewStub(::grpc::CreateCustomChannel(
  ...
  double end_to_end_time = 0;
	...
	// One-time rounds.
  std::tie(state_masked_x, client_masked_x_message) = 
                       gradient_descent_party_zero->
												  GenerateCorrelatedProductMessageForX().value();
  std::tie(state_masked_x_transpose, client_masked_x_transpose_message) = 
                       gradient_descent_party_zero->
												GenerateCorrelatedProductMessageForXTranspose().value();
	
	::grpc::ClientContext client_context_masked_x;
	client_message = GradientDescentClientMessage();
	server_message = GradientDescentServerMessage();
	*client_message.mutable_client_masked_x_message() = client_masked_x_message;
	if(!client_message.has_client_masked_x_message()){
		std::cerr << "Client: unexpected error: message doesn't have expected field" << std::endl;
		return 1;
	}
	grpc_status = stub->Handle(&client_context_masked_x,
	 client_message, &server_message);
	if (!grpc_status.ok()) {
				std::cerr << "Client: Failed on masked_x_message  with status " <<
					grpc_status.error_code() << " error_message: " <<
				  grpc_status.error_message() << std::endl;
				return 1;
			}
	server_masked_x_message = server_message.server_masked_x_message();
	...
	*(client_message.mutable_client_masked_x_transpose_message()) = client_masked_x_transpose_message;
	...
	for (int i = 0; i < num_iterations; i++) {
		...
		*client_message.mutable_start_message() = StartMessage();
		std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage> > rpc(stub->AsyncHandle(&client_context0, client_message, &cq));
		rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
		
  	 StateRound1 state_round_1;
  	 SigmoidInput sigmoid_input;
 		// Run round 1
 		
 		client_message = GradientDescentClientMessage();
 		std::tie(state_round_1, *client_message.mutable_client_gradient_descent_round_1_message()) =
 			gradient_descent_party_zero->GenerateGradientDescentRoundOneMessage(state_masked_x).value();
		...
		GPR_ASSERT(ok);
 		if (!grpc_status.ok()) {
 			std::cerr << "Client: Failed on message round 1 iteration " << i <<
 				" with status " <<
 				grpc_status.error_code() << " error_message: " <<
 			  grpc_status.error_message() << std::endl;
 			return 1;
   ...
   client_start = std::chrono::high_resolution_clock::now();
  	...
  	sigmoid_input = gradient_descent_party_zero->GenerateSigmoidInput(
  	                           state_round_1, server_masked_x_message,
  	 server_message.server_gradient_descent_round_1_message()).value();
	 
		::grpc::ClientContext client_context1;
		cq_index++;
		ok=false;
		 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context1, client_message, &cq));
		rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
	 ...
	 if (!use_new_mic) {
  	 ...
  	 applications::RoundFourSigmoidState sigmoid_round_4_state;
		...
		cq_index++;
		ok=false;
		 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context2, client_message, &cq));
		rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
		...
		cq_index++;
		ok=false;
		 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context3, client_message, &cq));
		rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
		...
		cq_index++;
		ok=false;
		 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context4, client_message, &cq));
		rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
		...
		cq_index++;
		ok=false;
		 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context5, client_message, &cq));
		rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
	...
	cq_index++;
	ok=false;
	 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context3_point_5, client_message, &cq));
	rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
	...
	cq_index++;
	ok=false;
	 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context6, client_message, &cq));
	rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
	...
	//////////////////////////////////////////////////////////////////////////////
	StateRound3 state_round_3;
	// Run final round ("Gradient Descent Round 3")
	client_start = std::chrono::high_resolution_clock::now();
	client_message = GradientDescentClientMessage();
	std::tie(state_round_3, *client_message.mutable_client_gradient_descent_final_message()) =
	  gradient_descent_party_zero->GenerateGradientDescentRoundThreeMessage(
	...
	cq_index++;
	ok=false;
	 rpc = std::unique_ptr<grpc::ClientAsyncResponseReader<GradientDescentServerMessage>>(stub->AsyncHandle(&client_context7, client_message, &cq));
	rpc->Finish(&server_message, &grpc_status, (void*)cq_index);
	
    Status status = gradient_descent_party_zero->ComputeGradientUpdate(
        state_round_3, server_masked_x_transpose_message, 
				server_message.server_gradient_descent_final_message());
    if (!status.ok()) {
...
int main(int argc, char** argv) {
  ...
  return private_join_and_compute::logistic_regression::ExecuteProtocol();
}
