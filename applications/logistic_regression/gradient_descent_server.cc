...
#include "applications/logistic_regression/gradient_descent_rpc_impl.h"
...
int RunServer() {
  ...
  for (size_t idx = 0; idx < gd_params.num_iterations; idx++) {
    ...
    auto matrix_shares_1 = 
                         SampleBeaverTripleMatrixBandC(
                             ...
                             gd_params.num_features, 1, 
														 gd_params.modulus).value();
    beaver_triple_matrix_b_c_round1_p0.push_back(matrix_shares_1.second);
    ...
    sigmoid_p0.push_back(std::move(preCompRes.second));
    ...
    auto matrix_shares_3 =
                         SampleBeaverTripleMatrixBandC(
                             ...
                             gd_params.num_examples, 1, 
														 gd_params.modulus).value();
    beaver_triple_matrix_b_c_round3_p0.push_back(matrix_shares_3.second);
  }
  ...
  std::unique_ptr<GradientDescentRpcImpl> service;
	if (use_new_mic) {
		service = std::make_unique<GradientDescentRpcImpl>(share_x, share_y, 			share_theta, std::move(share_provider), fpe_params, gd_params,
...
int main(int argc, char** argv) {
  ...
  return private_join_and_compute::logistic_regression::RunServer();
}
