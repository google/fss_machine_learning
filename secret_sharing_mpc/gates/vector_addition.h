...
#ifndef PRIVATE_JOIN_AND_COMPUTE_SECRET_SHARING_MPC_GATES_VECTOR_ADDITION_H_
#define PRIVATE_JOIN_AND_COMPUTE_SECRET_SHARING_MPC_GATES_VECTOR_ADDITION_H_
...
// The function in this file adds two vectors (also flattened matrices).
// Two players: P_0, P_1
...
StatusOr<std::vector<uint64_t>> VectorAdd(const std::vector<uint64_t>& vector_a,
                                          const std::vector<uint64_t>& vector_b,
                                          uint64_t modulus);

}  // namespace private_join_and_compute

#endif  // PRIVATE_JOIN_AND_COMPUTE_SECRET_SHARING_MPC_GATES_VECTOR_ADDITION_H_
